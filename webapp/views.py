...
def delete_event(request, event_id):
    try:
        ...
        # Ensure only the organiser can delete their event
        if request.user == event.organiser:
            event.delete()
            return redirect(reverse('webapp:categories', args=[event.category.name]))
        else:
            return HttpResponse("Unauthorised: You can only delete events you organised.")
    except Event.DoesNotExist:
        return HttpResponse("Event not found.")
=======
